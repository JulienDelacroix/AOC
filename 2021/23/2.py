...
def dfs(hallway, rooms, cost_so_far):
    ...
    if all(r == ([i] * 4) for i, r in enumerate(rooms)):
        return cost_so_far
    ...
    for h, amphipod in enumerate(hallway):
        if amphipod != -1 and all(val == amphipod for val in rooms[amphipod]):
            ...
            if all(hallway[p] == -1 for p in path):
                move_cost = amphipod_cost[amphipod] * (abs(rooms_x[amphipod] - hallway_x[h]) + (4 - len(rooms[amphipod])))
                hallway[h] = -1
        ...
        if len(room) and not all(v == r for v in room):
            for h in range(7):
                ...
                if hallway[h] == -1 and all(hallway[pos] == -1 for pos in path):
                    amphipod = room.pop()
                    hallway[h] = amphipod
                    move_cost = amphipod_cost[amphipod] * (abs(rooms_x[r] - hallway_x[h]) + (4 - (len(room))))
