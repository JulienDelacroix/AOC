...
for pattern_lines in sys.stdin.read().split("\n\n"):
    ...
    for x_axis in range(x_max-1):
        if sum(pattern[y][x1] != pattern[y][x2] for x1, x2 in symetricPairs(x_axis, x_max) for y in range(y_max)) == 1:
            res += (x_axis + 1)

    for y_axis in range(y_max-1):
        if sum(pattern[y1][x] != pattern[y2][x] for y1, y2 in symetricPairs(y_axis, y_max) for x in range(x_max)) == 1:
            res += (y_axis + 1) * 100
